...
from typing_extensions import Callable, NotRequired, TypedDict, Sequence
        ...
        **kwargs: Any,
        ...
        for layer in self.layers[:n_layers]:  # pyright: ignore
            if self.recompute_each_layer and x.requires_grad:
                x = torch.utils.checkpoint.checkpoint(partial(layer, **kwargs), x, use_reentrant=False)  # type: ignore
            else:
                x = layer(x, **kwargs)
...
class TabPFN2(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        output_dim = 10 if (n_classes is not None) else 5_000
        self.shuffle_labels = shuffle_labels and (n_classes is not None) and (2 < n_classes <= 10)
        ...
        state_dict = torch.load(
            lib.PROJECT_DIR / "checkpoints" / checkpoint,
            weights_only=True,
        )["state_dict"]
        ...
        self.decoder = nn.Sequential(
            nn.Linear(ninp, nhid),
            nn.GELU(),
            nn.Linear(nhid, output_dim),  # TODO handle this outside for regression
        )
    ...
    def forward(
        ...
        *,
        ...
        if shuffle_labels:
            labels_permutation = torch.randperm(self.n_classes, device=y_train.device)
            y_train_original_ohe = F.one_hot(y_train.to(torch.int64))
            y_train = labels_permutation[y_train.to(torch.int64)].to(torch.float32)
            assert torch.all(y_train_original_ohe == F.one_hot(y_train.to(torch.int64))[..., labels_permutation])
        ...
        preds = self.decoder(encoder_out[:, train_size:, -1])
        if self.n_classes is not None:
            preds = preds[..., :self.n_classes]

        # <<< Unshuffle labels
        if shuffle_labels:
            preds = preds[..., labels_permutation]
...
def preprocess_tabpfn_features(
        dataset: GraphDataset,
        config: Config,
        add_ratio_features_to = "num_features"
) -> np.ndarray:
    data = {
    ...
    if data["ratio_features"] is not None:
        ...
        if data[add_ratio_features_to] is None:
            data[add_ratio_features_to] = np.zeros((x_ratio.shape[0], 0), dtype=np.float32)
    ...
    # concat all features
    tabpfn_features_list = []
    for features_type in ["num_features", "cat_features"]:
        features = data.get(features_type, None)
        if features is not None:
            tabpfn_features_list.append(features)
    tabpfn_features = np.concatenate(tabpfn_features_list, axis=1)
        
    # z-scale all features
    tabpfn_features = (
        (tabpfn_features - tabpfn_features.mean(0))
        /
        tabpfn_features.std(0)
    )

    return tabpfn_features
...
def main(
    ...
    target_transform = preprocess_targets_results["target_transform"]
    
    tabpfn_features = preprocess_tabpfn_features(dataset, config)

    dataset = dataset.to_torch(device)
    tabpfn_features = torch.tensor(tabpfn_features, device=device)
    Y_train = (
    ...
    logger.info(f"Final: {dataset.n_num_features=} {dataset.n_ratio_features=} {dataset.n_cat_features=}")
    ...
    if "bins" in config:
        raise NotImplementedError(
            ...
            "Check both graph-mango-nova and tabpfn-finetune"
        )
    else:
        bin_edges = None
    ...
    n_classes = dataset.task.compute_n_classes() if dataset.task.is_classification else None

    tabpfn = TabPFN2(
        n_num_features=1,  # TODO: this is not actually used, refactor
        n_bin_features=0,
        cat_cardinalities=0,
        n_classes=n_classes,
    ...
    # Add trainable params for tabpfn
    all_param_names = [name for name, _ in tabpfn.named_parameters()]
    param_names = {
    ...
    if config.get("finetune_mode", "") == "none":
        ...
        params = lib.deep.make_parameter_groups(tabpfn)
    elif config.get("finetune_mode", "") == "":  # ?
        params = [
            p
            for pn, p in tabpfn.named_parameters()
        ...
        params = [
            p for p in tabpfn.transformer_encoder.layers[-finetune_layers:].parameters()
        ] + [p for p in tabpfn.decoder.parameters()]
    elif config.get("finetune_mode") == "full+ln":
        params = lib.deep.make_parameter_groups(tabpfn)
    elif config.get("finetune_mode", "") == "lora":
        assert config.get("lora_config", None) is not None
        lora_utils.replace_with_loras(tabpfn, config["lora_config"])  # type: ignore
        params = []
        for pn, p in tabpfn.named_parameters():
            if "lora" in pn:
                params.append(p)
    ...
    if dataset.task.is_regression:
        ...
        borders = torch.load(
            lib.PROJECT_DIR / "checkpoints" / "tabpfn-v2-regressor.ckpt",
            weights_only=True,
        ...
        pred_transform = regression_output_transform(
            ...
            loss_fn,
            renormalized_criterion,
            softmax_temperature=config["tabpfn"]["tabpfn_config"].get(
    ...
    def apply_model(part: PartKey, idx_train: Tensor, idx: Tensor) -> Tensor:
        tabpfn_input = [tabpfn_features]

        if "pearl" in config:
            tabpfn_input.append(pearl(dataset.data["graph"]))

        tabpfn_input = torch.cat(tabpfn_input, dim=-1)

        tabpfn_input = (
            (tabpfn_input - tabpfn_input.mean(0))
            /
            tabpfn_input.std(0)
        )
        x_num = torch.cat(
            [
                tabpfn_input[dataset.data["masks"]["train"]][idx_train],
                tabpfn_input[dataset.data["masks"][part]][idx],
            ],
            dim=1,
        )
        x_cat = None

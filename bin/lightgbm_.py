...
def main(
    ...
    if dataset.task.is_regression:
        dataset.data['labels'], regression_label_stats = lib.graph.data.standardize_labels(
    ...
    if dataset.data.get('num_features', None) is not None:
        ...
        X = {
            part: pd.DataFrame() for part in dataset.data['masks']
        }
    ...
    if dataset.data.get('ratio_features', None) is not None:
        ...
        X = {
            part: pd.concat([X[part], pd.DataFrame(dataset.data['ratio_features'][mask])], axis=1)
            for part, mask in dataset.data['masks'].items()
        }
        # Rename columns
        for part in X:
    ...
    if dataset.data.get('cat_features', None) is not None:
        ...
        categorical_features = range(dataset.n_num_features + dataset.n_ratio_features, dataset.n_features)
        X = {
            part: pd.concat([
                X[part],
                pd.DataFrame(dataset.data['cat_features'][mask], columns=categorical_features)
            ], axis=1)
            for part, mask in dataset.data['masks'].items()
        }
    ...
    if dataset.task.is_regression:
        ...
        if dataset.task.is_multiclass:
            ...
            fit_extra_kwargs = {
                'eval_metric': 'average_precision' if dataset.task.score == lib.Score.AP else 'binary_error'
            }

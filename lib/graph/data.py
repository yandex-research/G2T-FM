...
def load_data(
    ...
    if ratio_features is not None:
        ...
        logger.info(f"Transformed {mask.astype(int).sum()} feats to categorical")
        if mask.any().item():
            ...
            data['cat_features'] = cat_features
            data['ratio_features'] = None if mask.all().item() else ratio_features[:, ~mask]
...
def transform_num_features(
    ...
    if (policy is not None) and (NumPolicy(policy) != NumPolicy.NONE):
        ...
        elif policy in {NumPolicy.NOISY_QUANTILE_NORMAL, NumPolicy.NOISY_QUANTILE_UNIFORM}:
...
def shuffle_categories(
    ...
    for i in range(cat_features.shape[1]):
        ...
        shuffle = {
            categories[j]: permuted_categories[j]
...
def apply_nfa(
    ...
    if dataset.data[f'num_features'] is not None:
        for mode in num_modes:
            nfa['num_features'].append(
                _nfa_reduce(graph, dataset.data['num_features'], mode=mode, weights=weights)
            )

    if dataset.data['ratio_features'] is not None:
        nfa['ratio_features'].append(
            _nfa_reduce(graph, dataset.data['ratio_features'], mode='mean', weights=weights)
        )
    ...
    if dataset.data['cat_features'] is not None:
        ...
        nfa_cat_features = _nfa_reduce(graph, cat_features, mode='mean', weights=weights)
...
def apply_pca(
    ...
    logger.warning(f"Applying PCA to {total_n_featuers} features")
    ...
    features = transform_num_features(
        features, dataset.data['masks'],
        dataset.task.setting, postprocess, seed=42,
    )
...
def compute_graph_encodings(
    ...
    if preporcessing_policy is not None:
        graph_encodings = transform_num_features(
            graph_encodings, dataset.data["masks"], dataset.task.setting,
            policy=preporcessing_policy, seed=42,
        )
    
    return {
        'num_features': graph_encodings
    }
...
def build_dataset(
    ...
    **dataset_params,
    ...
    original_features = {
        key: dataset.data.pop(key, None) for key in feature_types
    }

    for features in [original_features, nfa_features]:
        features['num_features'] = transform_num_features(
            features.get('num_features', None), dataset.data['masks'],
            setting, num_policy, seed
        )
        features['ratio_features'] = transform_num_features(
            features.get('ratio_features', None), dataset.data['masks'],
            setting, ratio_policy, seed
        )
        features['cat_features'] = transform_cat_features(
            features.get('cat_features', None), dataset.data['masks'],
            setting, cat_policy
        )
    ...
    merged_features = merge_features(
        [original_features, nfa_features, graph_encodings]
    )
